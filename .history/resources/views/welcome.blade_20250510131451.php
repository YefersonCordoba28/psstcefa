...
<!-- Encabezado profesional y armonioso -->
<header class="fixed top-0 w-full bg-white border-b border-orange-300 shadow-sm z-50">
  <div class="max-w-7xl mx-auto px-4 sm:px-6 py-3 flex items-center justify-between">

    <!-- Logo + Título -->
    <div class="flex items-center gap-4">
      <img src="{{ asset('images/logosena.png') }}" alt="Logo SENA" class="w-12 h-12 md:w-14 md:h-14 bg-white p-1 rounded-full shadow-md" />
      <div>
        <h1 class="text-xl md:text-2xl font-bold text-orange-600 leading-tight">Sistema de Gestión SST</h1>
        <p class="text-sm md:text-base text-gray-600 font-light">Seguridad, salud y tecnología en acción</p>
      </div>
    </div>

    <!-- Botón hamburguesa móvil -->
    <button id="menu-toggle" class="md:hidden text-3xl text-orange-600 focus:outline-none">
      <i class="bi bi-list"></i>
    </button>

    <!-- Menú principal -->
    <nav id="menu" class="hidden md:flex flex-col md:flex-row md:items-center gap-4 md:gap-6 text-sm font-medium absolute md:static top-20 right-4 bg-white md:bg-transparent shadow md:shadow-none p-4 md:p-0 rounded-md md:rounded-none transition-all duration-300 ease-in-out text-orange-600">
      @if (Route::has('login'))
        <a class="hover:text-orange-800 transition" href="{{ route('login') }}">Login</a>
      @endif
      <a href="#" class="hover:text-orange-800 transition">Reportes</a>
      <a href="#" class="hover:text-orange-800 transition">Contacto</a>
    </nav>
  </div>
</header>
