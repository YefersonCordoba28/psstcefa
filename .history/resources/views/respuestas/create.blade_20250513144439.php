    ...
    <div class="bg-dark p-4 rounded shadow-lg border border-info">
        ...
        <form action="{{ route('respuestas.store') }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <select name="tipo_evento" id="tipo_evento" class="form-select bg-secondary text-white" required onchange="mostrarEventos()">
            ...
            <div class="mb-3" id="evento_id_container" style="display: none;">
                ...
                <select name="evento_id" id="evento_id" class="form-select bg-secondary text-white" required onchange="mostrarDetalles()">
            ...
            <div class="mb-3">
                ...
                <textarea name="respuesta" class="form-control bg-secondary text-white" rows="3">{{ old('respuesta') }}</textarea>
            </div>

            <div class="mb-3">
                <label for="acciones_tomadas" class="form-label">Acciones Tomadas</label>
                <textarea name="acciones_tomadas" class="form-control bg-secondary text-white" rows="3">{{ old('acciones_tomadas') }}</textarea>
            </div>

            <div class="mb-3">
                <label for="fecha_respuesta" class="form-label">Fecha de Respuesta</label>
                <input type="datetime-local" name="fecha_respuesta" class="form-control bg-secondary text-white" required>
            </div>

            <div class="mb-3">
                <label for="respondido_por" class="form-label">Respondido por</label>
                <input type="text" name="respondido_por" class="form-control bg-secondary text-white" value="{{ old('respondido_por') }}">
            </div>

            <div class="d-flex justify-content-between">
                <button type="submit" class="btn btn-success px-4"><i class="bi bi-save"></i> Guardar</button>
                <a href="{{ route('respuestas.index') }}" class="btn btn-outline-light px-4"><i class="bi bi-x-circle"></i> Cancelar</a>
            </div>
        </form>
    </div>
</div>

<!-- Bootstrap Icons (si no están cargados ya) -->
