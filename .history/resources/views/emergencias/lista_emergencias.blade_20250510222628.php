...
@section('content')
    ...
    <div class="card-header bg-dark text-white">
        <h5 class="card-title m-0">
            <i class="fas fa-chart-bar me-2"></i>Estadísticas de Emergencias por Mes ({{ date('Y') }})
        </h5>
    </div>
    <div class="card-body">
        <canvas id="emergenciasChart" height="100"></canvas>
    </div>
</div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const meses = {!! json_encode(array_column($emergenciasPorMes->toArray(), 'nombre_mes')) !!};
        const totales = {!! json_encode(array_column($emergenciasPorMes->toArray(), 'total')) !!};
