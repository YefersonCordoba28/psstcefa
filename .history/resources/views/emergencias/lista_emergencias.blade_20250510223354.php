   ...
   <div class="card shadow-sm mt-4">
    <div class="card-header bg-dark text-white d-flex justify-content-between align-items-center">
        ...
        <select id="yearSelector" class="form-select form-select-sm" style="width: 120px;">
            @foreach($datosGrafica['años'] as $year)
                <option value="{{ $year }}" {{ $year == $datosGrafica['añoSeleccionado'] ? 'selected' : '' }}>
                    {{ $year }}
                </option>
            @endforeach
        </select>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const chartConfig = {
            type: 'line',
            data: {
                labels: datosGrafica.meses,
                datasets: [{
                    label: 'Emergencias',
                    data: datosGrafica.datos[datosGrafica.añoSeleccionado],
        ...
        document.getElementById('yearSelector').addEventListener('change', function() {
            ...
            emergenciasChart.data.datasets[0].data = datosGrafica.datos[selectedYear];
