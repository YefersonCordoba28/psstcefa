...
<!-- Header mejorado -->
<header class="fixed top-0 w-full bg-indigo-600 text-white shadow-lg z-50">
  <div class="max-w-7xl mx-auto px-4 sm:px-6 py-4 flex items-center justify-between">
    ...
    <!-- Menú principal -->
    <nav id="menu" class="hidden md:flex flex-col md:flex-row md:items-center gap-4 md:gap-6 text-sm font-medium absolute md:static top-20 right-4 bg-indigo-600 md:bg-transparent shadow md:shadow-none p-4 md:p-0 rounded-md md:rounded-none">
      @if (Route::has('login'))
        <a class="hover:text-orange-300 transition" href="{{ route('login') }}">{{ __('Login') }}</a>
      @endif
      <a href="#" class="hover:text-orange-300 transition">Reportes</a>
      <a href="#" class="hover:text-orange-300 transition">Contacto</a>
    </nav>
  </div>
</header>
