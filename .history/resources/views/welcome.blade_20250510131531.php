...
<!-- Header mejorado en naranja -->
<header class="fixed top-0 w-full bg-orange-500 text-white shadow-lg z-50">
  <div class="max-w-7xl mx-auto px-4 sm:px-6 py-4 flex items-center justify-between">
    <div class="flex items-center gap-3">
      <img src="{{ asset('images/logosena.png') }}" alt="Logo" class="w-10 h-10 md:w-12 md:h-12 object-contain bg-white rounded-full p-1" />
      <h1 class="text-lg md:text-2xl font-bold tracking-wide">Sistema de Gestión SST</h1>
    </div>
    ...
    <!-- Menú principal -->
    <nav id="menu" class="hidden md:flex flex-col md:flex-row md:items-center gap-4 md:gap-6 text-sm font-medium absolute md:static top-20 right-4 bg-orange-500 md:bg-transparent shadow md:shadow-none p-4 md:p-0 rounded-md md:rounded-none">
      @if (Route::has('login'))
        <a class="hover:text-white/80 transition" href="{{ route('login') }}">{{ __('Login') }}</a>
      @endif
      <a href="#" class="hover:text-white/80 transition">Reportes</a>
      <a href="#" class="hover:text-white/80 transition">Contacto</a>
    </nav>
  </div>
</header>
