...
<!-- Header profesional con diseño formal y atractivo -->
<header class="fixed top-0 w-full bg-[#1F2A44] text-white shadow-lg z-50">
  <div class="max-w-7xl mx-auto px-4 sm:px-6 py-5 flex items-center justify-between">
    <!-- Logo + Título -->
    <div class="flex items-center gap-5">
      <img src="{{ asset('images/logosena.png') }}" alt="Logo SENA" class="w-14 h-14 md:w-16 md:h-16 bg-white p-1.5 rounded-full shadow-md transition-transform duration-300 hover:scale-105" />
      <div>
        <h1 class="text-2xl md:text-3xl font-bold leading-tight tracking-wide">Sistema de Gestión SST</h1>
        <p class="text-sm md:text-base font-light opacity-90">Seguridad, salud y tecnología en acción</p>
      </div>
    </div>
    ...
    <!-- Menú principal -->
    <nav id="menu" class="hidden md:flex flex-col md:flex-row md:items-center gap-6 text-sm font-medium absolute md:static top-24 right-4 bg-[#1F2A44] md:bg-transparent shadow-lg md:shadow-none p-5 md:p-0 rounded-lg md:rounded-none transition-all duration-300 ease-in-out">
      @if (Route::has('login'))
        <a class="relative text-white hover:text-blue-200 transition-colors duration-300 hover:underline underline-offset-4" href="{{ route('login') }}">{{ __('Login') }}</a>
      @endif
      <a href="#" class="relative text-white hover:text-blue-200 transition-colors duration-300 hover:underline underline-offset-4">Reportes</a>
      <a href="#" class="relative text-white hover:text-blue-200 transition-colors duration-300 hover:underline underline-offset-4">Contacto</a>
    </nav>
  </div>
</header>
