...
@section('content')
                    ...
                    <div class="card shadow-sm">
                        ...
                        <div class="card-body p-4">
                            ...
                            <div class="accidente-list d-md-none">
                                @forelse ($accidentes as $accidente)
                                    <div class="card mb-3 accidente-item" data-gravedad="{{ $accidente->gravedad }}">
                                        <div class="card-body p-3">
                                            <div class="row">
                                                <div class="col-5"><strong>Identificación</strong></div>
                                                <div class="col-7">{{ $accidente->id }}</div>
                                            </div>
                                            <div class="row">
                                                <div class="col-5"><strong>Fecha y hora</strong></div>
                                                <div class="col-7">{{ $accidente->fecha_hora }}</div>
                                            </div>
                                            <div class="row">
                                                <div class="col-5"><strong>Área</strong></div>
                                                <div class="col-7">{{ $accidente->areaUnidadProductiva->nombre ?? 'N/A' }}</div>
                                            </div>
                                            <div class="row">
                                                <div class="col-5"><strong>Tipo de lesión</strong></div>
                                                <div class="col-7">{{ $accidente->tipoLesion->nombre ?? 'N/A' }}</div>
                                            </div>
                                            <div class="row">
                                                <div class="col-5"><strong>Tipo de riesgo</strong></div>
                                                <div class="col-7">{{ $accidente->tipoRiesgo->nombre ?? 'N/A' }}</div>
                                            </div>
                                            <div class="row">
                                                <div class="col-5"><strong>Tipo de accidente</strong></div>
                                                <div class="col-7">{{ $accidente->tipoAccidente->nombre ?? 'N/A' }}</div>
                                            </div>
                                            <div class="row">
                                                <div class="col-5"><strong>Gravedad</strong></div>
                                                <div class="col-7">
                                                    <span class="badge {{ $accidente->gravedad === 'leve' ? 'bg-success' : ($accidente->gravedad === 'moderada' ? 'bg-warning' : ($accidente->gravedad === 'grave' ? 'bg-danger' : 'bg-dark')) }}">
                                                        {{ ucfirst($accidente->gravedad) }}
                                                    </span>
                                            ...
                                            <div class="row">
                                                <div class="col-5"><strong>Evidencias</strong></div>
                                                <div class="col-7">
                                                    @if($accidente->evidencia)
                                                        <img src="{{ asset('img_accidentes/' . basename($accidente->evidencia)) }}" 
                                                             alt="Evidencia" 
                                ...
                                @empty
                                    <div class="text-center py-3">No hay accidentes registrados.</div>
                                @endforelse
                            </div>
                        </div>
                    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const accidenteItems = document.querySelectorAll('.accidente-item');
            
            // Función para filtrar la tabla y las tarjetas
            function filterItems() {
                const selectedGravedad = gravedadFilter.value.toLowerCase();
                
                // Filtrar la tabla (desktop)
                ...
                // Filtrar las tarjetas (mobile)
                accidenteItems.forEach(item => {
                    const itemGravedad = item.getAttribute('data-gravedad').toLowerCase();
                    
                    if (selectedGravedad === '' || itemGravedad === selectedGravedad) {
                        item.style.display = '';
                    } else {
    ...
    <style>
        ...
        #accidentesTable th:nth-child(1),
        #accidentesTable td:nth-child(1) { /* Identificación */
            width: 8%;
        }
        
        #accidentesTable th:nth-child(2),
        #accidentesTable td:nth-child(2) { /* Fecha y hora */
            width: 15%;
        }
        
        #accidentesTable th:nth-child(3),
        #accidentesTable td:nth-child(3) { /* Área */
            width: 10%;
        }
        
        #accidentesTable th:nth-child(4),
        #accidentesTable td:nth-child(4) { /* Tipo de lesión */
            width: 12%;
        }
        
        #accidentesTable th:nth-child(5),
        #accidentesTable td:nth-child(5) { /* Tipo de riesgo */
            width: 12%;
        }
        
        #accidentesTable th:nth-child(6),
        #accidentesTable td:nth-child(6) { /* Tipo de accidente */
            width: 12%;
        }
        
        #accidentesTable th:nth-child(7),
        #accidentesTable td:nth-child(7) { /* Gravedad */
            width: 8%;
        }
        
        #accidentesTable th:nth-child(8),
        #accidentesTable td:nth-child(8) { /* Evidencias */
            width: 10%;
        }
        
        #accidentesTable th:nth-child(9),
        #accidentesTable td:nth-child(9) { /* Acciones */
            width: 13%;
        }
        ...
        /* Estilos para las tarjetas (Mobile) */
        .accidente-list .card {
        ...
        .accidente-list .card .row {
            margin-bottom: 0.5rem;
        }
        
        .accidente-list .card .row div {
            padding: 0.25rem;
        }
        
        .accidente-list .card .row .col-5 {
            font-weight: bold;
            background-color: #e9ecef;
            border-right: 1px solid #ddd;
        }
        
        .accidente-list .card .row .col-7 {
            padding-left: 0.5rem;
        }
        
        .accidente-list .card .evidence-img {
            transition: all 0.3s ease;
            cursor: pointer;
        }
        
        .accidente-list .card .evidence-img:hover {
