...
@section('contenido') <!-- Aquí empieza la sección de contenido -->
        ...
        <table class="table table-sm table-bordered">
            ...
            <tbody>
                @foreach ($datos as $item)
                    <tr>
                        <td><center>{{ $evento->id }}</center></td>
                        <td><center>{{ $evento->fecha }}</center></td>
                        <td><center>{{ $evento->hora }}</center></td>
                        <td><center>{{ $evento->ubicación }}</center></td>
                        <td><center>{{ $evento->tipo_evento }}</center></td>
                        <td><center>{{ $evento->descripción_evento }}</center></td>
                        <td>
                            <center><button type="button" class="btn btn-success editbtn" data-id="{{ $evento->id }}" data-fecha="{{ $evento->fecha }}" data-hora="{{ $evento->hora }}" data-ubicación="{{ $evento->ubicación }}" data-tipo_evento="{{ $evento->tipo_evento }}" data-descripción_evento="{{ $evento->descripción_evento }}" data-bs-toggle="modal" data-bs-target="#editar">Editar</button></center>
                        </td>
                        <td>
                            <center><button type="button" class="btn btn-danger deletebtn" data-id="{{ $evento->id }}" data-bs-toggle="modal" data-bs-target="#eliminar">Eliminar</button></center>
                        </td>
                    </tr>
                @endforeach
            </tbody>
                    ...
                    <form id="formEditar" action="{{ route('eventos.update', 0) }}" method="POST">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="mb-3">
                                ...
                                <textarea class="form-control" id="edit-descripción_evento" name="descripción_evento"></textarea>
                            </div>
                        </div>
