...
<!-- Header profesional con un diseño formal y atractivo -->
<header class="fixed top-0 w-full bg-gradient-to-r from-orange-600 via-orange-500 to-orange-600 text-white shadow-lg z-50">
  <div class="max-w-7xl mx-auto px-4 sm:px-6 py-4 flex items-center justify-between">
    <!-- Logo + Título -->
    <div class="flex items-center gap-4">
      <img src="{{ asset('images/logosena.png') }}" alt="Logo SENA" class="w-12 h-12 md:w-14 md:h-14 bg-white p-1 rounded-full shadow-md transition-transform duration-300 hover:scale-105" />
      <div>
        <h1 class="text-xl md:text-2xl font-semibold leading-tight tracking-wide">Sistema de Gestión SST</h1>
        <p class="text-sm md:text-base font-light opacity-80">Seguridad, salud y tecnología en acción</p>
      </div>
    </div>

    <!-- Menú hamburguesa para móviles -->
    <button id="menu-toggle" class="md:hidden text-3xl focus:outline-none transition-transform duration-300 hover:scale-110">
      <i class="bi bi-list"></i>
    </button>

    <!-- Menú principal -->
    <nav id="menu" class="hidden md:flex flex-col md:flex-row md:items-center gap-4 md:gap-6 text-sm font-medium absolute md:static top-20 right-4 bg-orange-600 md:bg-transparent shadow-lg md:shadow-none p-4 md:p-0 rounded-md md:rounded-none transition-all duration-300 ease-in-out">
      @if (Route::has('login'))
        <a class="relative text-white hover:text-orange-200 transition-colors duration-300 hover:underline underline-offset-4" href="{{ route('login') }}">{{ __('Login') }}</a>
      @endif
      <a href="#" class="relative text-white hover:text-orange-200 transition-colors duration-300 hover:underline underline-offset-4">Reportes</a>
      <a href="#" class="relative text-white hover:text-orange-200 transition-colors duration-300 hover:underline underline-offset-4">Contacto</a>
    </nav>
  </div>
</header>
